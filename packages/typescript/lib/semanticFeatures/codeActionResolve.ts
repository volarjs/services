...
export function resolveFixAllCodeAction(
	...
	const fixes = data.fixIds.map(fixId =>
		safeCall(() =>
			ctx.languageService.getCombinedCodeFix(
				{ type: 'file', fileName: data.fileName },
				fixId,
				formatOptions,
				preferences,
			)
...
export function resolveRefactorCodeAction(
	...
	const editInfo = safeCall(() =>
		ctx.languageService.getEditsForRefactor(
			data.fileName,
			formatOptions,
			data.range,
			data.refactorName,
...
export function resolveOrganizeImportsCodeAction(
	...
	const changes = safeCall(() =>
		ctx.languageService.organizeImports({ type: 'file', fileName: data.fileName }, formatOptions, preferences)
	);
	codeAction.edit = convertFileTextChanges(changes ?? [], ctx.fileNameToUri, ctx.getTextDocument);
}
