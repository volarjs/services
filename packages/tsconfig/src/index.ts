import type { DocumentLink, FileType, ServicePluginInstance, ServicePlugin } from '@volar/language-service';
		...
		create(context): ServicePluginInstance {
			...
			async function resolveNodeModulesPath(baseDirUri: URI, pathCandidates: string[]): Promise<URI | undefined> {
				...
				while (true) {
					...
					const moduleStat = await context.env.fs?.stat(moduleAbsoluteUrl.toString());
			...
			*/
			...
			async function exists(resource: URI): Promise<boolean> {
				const stat = await context.env.fs?.stat(resource.toString());
